...
static void emitReturn() {
  // If we are in an init function we emit this instead of NIL
...
// We look through the local scope array, if we don't find the variable
// We assume it's a global variable and return -1
 ...
 * certain precedence level or higher.*/
...
static void forStatement() {
  // If for loop defines a var that should be in the loop scope
  beginScope();
...
static void whileStatement() {
  // store the current stack position as the start of the loop
  int loopStart = currentChunk()->count;
  ...
  // loopStart is the position of the while loop, this will be repeated until
  // JUMP IF FALSE is emitted i.e. the expression is falsey
  emitLoop(loopStart);
